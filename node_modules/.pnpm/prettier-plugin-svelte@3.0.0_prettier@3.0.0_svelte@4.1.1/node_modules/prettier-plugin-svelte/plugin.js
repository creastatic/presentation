    ...
    switch (node.type) {
        ...
        case 'AwaitBlock': {
            ...
            if (!hasPendingBlock && hasThenBlock) {
                ...
                if (hasThenBlock) {
                    block.push(group(['{:then', expandNode(node.value), '}']), path.call(print, 'then'));
                }
            }
            if ((hasPendingBlock || hasThenBlock) && hasCatchBlock) {
                block.push(group(['{:catch', expandNode(node.error), '}']), path.call(print, 'catch'));
            }
            block.push('{/await}');
            return group(block);
        }
...
function embed(path, _options) {
    ...
    switch (parent.type) {
        ...
        case 'EachBlock':
            ...
            break;
        case 'Element':
            printJS$1(parent, (_a = options.svelteStrictMode) !== null && _a !== void 0 ? _a : false, false, false, 'tag');
            break;
        case 'MustacheTag':
